...
- This is a Bun/Turborepo workspace with the deployable package `@apps/server`, the reusable `@socketo/realtime-core` package, and the Node `@socketo/cli` package. Cloudflare application code is under `apps/server/`.
...
- Run all workspace development tasks with `bun run dev`; run only the Worker with `bun run --filter=@apps/server dev`.
- Build everything with `bun run build`; build only the Worker with `bun run --filter=@apps/server build`.
- Build the shared core with `bun run --filter=@socketo/realtime-core build` and the CLI with `bun run --filter=@socketo/cli build`.
- Run unit tests with `bun test` or `bun run --filter=@socketo/realtime-core test` (10 unit tests, 126 assertions).
- Start the local CLI with `bun run --filter=@socketo/cli start`; it is a standalone Node `ws` adapter over `@socketo/realtime-core`.
- Deploy only through `bun run --filter=@apps/server deploy`; this runs `vite build` before `wrangler deploy`.
- Regenerate Cloudflare bindings after changing `apps/server/wrangler.jsonc` with `bun run --filter=@apps/server cf-typegen`; `apps/server/worker-configuration.d.ts` is generated and should not be edited manually.
- Run `bunx oxlint .` for the configured Oxlint rules; `bunx tsc -p apps/server/tsconfig.json --noEmit` is the focused typecheck.

## Local Worker Setup

- Start the Worker before initializing local data: `bun run --filter=@apps/server dev`.
...
- Socket IDs generated across all WebSocket connections follow the official Pusher Channels standard `<int>.<int>` (`generateSocketId()` from `@socketo/realtime-core`).
...
- Keep Worker/runtime-specific code out of reusable protocol logic. `@socketo/realtime-core` owns protocol state, socket ID generation, and validation; the Durable Object and CLI packages provide their respective WebSocket adapters.
